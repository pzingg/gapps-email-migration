...
# mb2md [-c] -s sourcedir [-l wu-mailboxlist] [-R|-f somefolder] [-d destdir] [-r strip_extension] [-a]
#
#  -a            read an Apple Mail 2.0 emlx-style tree 
#                Implies '-R'
#                Incompatible with '-f'
#
...
sub usage() {
    ...
    die   "       mb2md [-c] -s sourcedir [-l wu-mailboxlist] [-R|-f somefolder] [-d destdir] [-r strip_extension] [-a]\n";
}
                    # get options
my %opts;
getopts('d:f:achms:r:l:R', \%opts) || usage();
...
else
{
	...
	while (my $sourcefile = readdir(SDIR))
	{
		...
		elsif (!defined($apple_mail)) {
			if (!-f "$mbroot/$sourcefile")
			{
...
sub convertit
{
	...
	if (defined($apple_mail)) {
		$destinationdir =~ s/\.(imapmbox|mbox)$//;
	} elsif (defined($strip_ext)) {
		$destinationdir =~ s/\.$strip_ext$//;
	}
	
	# Appending $oldpath => path is only missing $dest
	if (defined($apple_mail)) {
		$destinationdir = "$oldpath/$destinationdir" if ($oldpath ne "");
	} else {
		my $temppath = $oldpath;
	...
	if (-d $srcdir) {
		if (defined($apple_mail)) {
			my $messages_dir = "$srcdir/Messages";
			if (-d $messages_dir) {
				opendir(SUBDIR, "$messages_dir") or die "can't open $messages_dir !\n";
				my @msglist=readdir(SUBDIR);
				closedir(SUBDIR);
				foreach (@msglist) {
					next unless (/^([0-9]+)\.emlx$/);
					&convert_emlx("$messages_dir/$_","$dest/$destinationdir",$1);
				}
			}
		}
		...
		closedir(SUBDIR);
		foreach (@subdirlist) {
			next if (/^\.+$/ || (defined($apple_mail) && (!-d "$srcdir/$_" || /^(Other Users|Messages)$/)));
			my $newpath;
			if ($oldpath eq "") {
				$newpath = $dir;
			} else {
...
sub convert
{
	...
	&do_convert($mbox, $maildir, 0, undef);
}

sub convert_emlx
{
	# get the source and destination as arguments
	my ($emlx, $maildir, $messageid) = @_;
	
	printf("Source emlx file is $emlx\n");
    printf("Target Maildir is $maildir \n");
	&do_convert($emlx, $maildir, 1, $messageid);
}
...
{
	# get the source and destination as arguments
	my ($mbox, $maildir, $is_emlx, $messageid) = @_;
	...
	while(<MBOX>)
	{
		...
		{
			$pos = tell(MBOX);
			$in_emlx_length_line = 0;
			if (!defined $emlx_limit) {
				if (/^([0-9]+)\s*$/) {
					$in_emlx_length_line = 1;
					$emlx_limit = $pos + $1;
				} else {
					printf("invalid emlx file: no message length\n");
					return;
				}
			} 
		}
			
	    if ( ($in_emlx_length_line || /^From /)
			...
			&& (!defined $contentlength) )
	    {
			...
			if ($messagecount > 0)
			{
			    ...
			    if ($messagefn ne '') {
					...
					utime $t, $t, $messagefn if defined($t);
					$receivedate = 'Bogus';
				}
			}
			...
			&& !($in_emlx_length_line || /^From /) )
	    {
			...
			if ($_ eq "\n")
			{
				...
				$messageid = $messagecount unless defined($messageid);
				if (defined($apple_mail)) {
					$messagefn = sprintf ("%d.%06d,", $unique, $messageid);
				} else {
					$messagefn = sprintf ("cur/%d.%06d.mbox:2,", $unique, $messageid);
				}
				undef $messageid;
		...
		last if ($is_emlx && ($pos >= $emlx_limit));
